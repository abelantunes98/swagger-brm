...
@Validated
public interface GlidApi {

    @Operation(summary = "Get G/L IDs", description = "Gets the general ledger IDs (G/L IDs) that match the specified query criteria.", tags={ "Installments" })
    @ApiResponses(value = { 
        @ApiResponse(responseCode = "200", description = "The G/L IDs were returned successfully.", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = GLIDSpec.class)))),
    ...
    @RequestMapping(value = "/glid",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<List<GLIDSpec>> getGLID(@Parameter(in = ParameterIn.QUERY, description = "The G/L ID." ,schema=@Schema()) @Valid @RequestParam(value = "glid", required = false) String glid, @Parameter(in = ParameterIn.QUERY, description = "The description." ,schema=@Schema()) @Valid @RequestParam(value = "descr", required = false) String descr);
