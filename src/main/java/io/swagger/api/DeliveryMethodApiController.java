...
@RestController
public class DeliveryMethodApiController implements DeliveryMethodApi {

    private static final Logger log = LoggerFactory.getLogger(DeliveryMethodApiController.class);
    ...
    public ResponseEntity<List<DeliveryMethod>> getDeliveryMethod(@Parameter(in = ParameterIn.QUERY, description = "The delivery method ID." ,schema=@Schema()) @Valid @RequestParam(value = "id", required = false) String id,@Parameter(in = ParameterIn.QUERY, description = "Search by name" ,schema=@Schema()) @Valid @RequestParam(value = "name", required = false) String name,@Min(0)@Parameter(in = ParameterIn.QUERY, description = "The position from where the fetch records must start." ,schema=@Schema(allowableValues={  }
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<List<DeliveryMethod>>(objectMapper.readValue("[ {\n  \"name\" : \"name\",\n  \"id\" : \"id\",\n  \"status\" : \"ACTIVE\"\n}, {\n  \"name\" : \"name\",\n  \"id\" : \"id\",\n  \"status\" : \"ACTIVE\"\n} ]", List.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<List<DeliveryMethod>>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<List<DeliveryMethod>>(HttpStatus.NOT_IMPLEMENTED);
    }
